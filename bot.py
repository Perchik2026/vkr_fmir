...
class BotHandler:
    ...
    def handle(self, user_id, text, first_name, last_name):
        ...
        if text == "СОДЕРЖАНИЕ":
            self.send_message(user_id, "ВКР состоит из: 
                                        Титульный лист
                                        Введение
                                        ...
                                        Основная часть ВКР бакалавра состоит из трех глав. Названия отдельных глав должны быть сопряжены с темой работы, а названия параграфов согласовываются с названиями соответствующих глав (но не совпадают с ними!).
                                        Главы ВКР разделяются на параграфы, четко обозначив, тем самым, вопросы, исследуемые в рамках каждой главы. Все главы должны быть сбалансированы и пропорциональны, приблизительного одного объема. В среднем глава состоит из 2–3 параграфов.
                                        ❗️В связи с тем, что параграфы также должны быть приблизительно одного объёма, рекомендовано разделять главы на 3 параграфа.
                                        
                                        Выберите пункт:", create_vkr_content_keyboard())
            return
